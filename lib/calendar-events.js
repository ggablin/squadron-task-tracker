...
async function seedIfEmpty(client, defaults = DEFAULTS) {
  const { rows } = await client.query('SELECT COUNT(*)::int AS n FROM calendar_events');
...
const realDate = (s) => {
  ...
  // Guard before toISOString: an Invalid Date (e.g. '2026-13-01') would throw.
...
module.exports = { STATUSES, DDL, ensureTable, seedIfEmpty, listAll, get, validate, create, update, remove };
