...
// The one place the default rows are written; see lib/duties.js.
...
// The seed.js path; see lib/duties.js for why this exists, why it tests for
// empty rather than absent, and why it must run on seed.js's own client.
async function seedIfEmpty(client, defaults = DEFAULTS) {
  const { rows } = await client.query('SELECT COUNT(*)::int AS n FROM drill_dates');
...
module.exports = {
  ...
  DDL, ensureTable, seedIfEmpty, listAll, get, findOverlap, create, update, remove,
};
