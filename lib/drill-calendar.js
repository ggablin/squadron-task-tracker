...
// Date | 'YYYY-MM-DD…' → 'YYYY-MM-DD'. A to_char'd column or a request body arrives
// as a string and passes through unchanged. A bare pg DATE column arrives as a Date
// at LOCAL midnight — node-postgres does NOT apply UTC — so reading it back with
// toISOString() (which is UTC) can land on the previous day in any zone ahead of
// UTC. Prefer a to_char'd string from Postgres when the exact date matters; this
// branch exists for callers that only have a bare Date to hand.
function isoDate(v) {
  if (v instanceof Date) return v.toISOString().slice(0, 10);
  return String(v == null ? '' : v).slice(0, 10);
}

// Is `s` a real 'YYYY-MM-DD' calendar date? The single implementation — the
// drill routes reach it through parseIso below, the event routes import it
// directly (lib/calendar-events.js).
//
// Both halves are load-bearing. The regex only checks the SHAPE, so '2026-02-30'
// passes it and then parses in JS as 2 March — the round-trip catches that. And
// '2026-13-01' passes the regex too but yields an Invalid Date, whose
// toISOString() THROWS rather than returning a mismatched string, so the
// Number.isNaN guard has to come first. calendar-events.js once kept its own
// copy of this and lost exactly that guard: the throw escaped an async handler
// and hung the request for 305 seconds. One copy now, so it cannot drift again.
...
function parseIso(s) {
  return isIsoCalendarDate(s) ? new Date(s + 'T00:00:00Z') : null;
}
...
// `ref` may be absent — a cycle created through the Task Builder carries no
// start_date, and the caller has nothing to date the calendar against. Marking
// relative to today instead is the tempting fallback and the wrong one: it made the
// August deck strike through its own drill and bold September. Empty-string is no
// safer, because every real date sorts after it and January silently becomes "next".
// With no reference the calendar still lists every drill, it just claims nothing
// about which one has been and which one is coming.
function drillEntries(all, year, ref) {
  const rows = all.filter(r => r.start_date.slice(0, 4) === String(year));
  let nextSeen = false;
  return rows.map(d => {
    if (!ref) return { kind: 'drill', ...d, label: label(d.start_date, d.end_date),
                       threeDay: dayCount(d.start_date, d.end_date) >= 3, past: false, next: false };
...
module.exports = {
  isoDate, isIsoCalendarDate, label, years, buildYear, buildCalendar, validateDrill, overlaps,
