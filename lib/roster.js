...
async function updateMember(db, id, input, actorId) {
  ...
  try {
    ...
    // JSON `0` instead of `false`. Strict now, not coerced: any non-boolean
    // `active` is rejected outright rather than silently reinterpreted. The
    // library owns this invariant regardless of caller — it must not depend
    // on the HTTP boundary's parseStrictBool (server.js) having already
    // sanitised the value, since this exact guard/write mismatch produced the
    // only empirical account lockout found in this codebase.
    let activeVal;
    if (input.active !== undefined) {
      if (typeof input.active !== 'boolean') {
        throw new RosterError('active must be true or false', 'BAD_ACTIVE');
      }
    ...
    for (const col of EDITABLE) {
      ...
      if (col === 'shop_id') {
        ...
        // Match createMember's `input.email || null`: an explicit null and
        // an empty string both mean "no email," so both must store SQL NULL.
        // Before this, createMember stored NULL for '' while updateMember
        // (falling into the generic branch below) stored the literal string
        // '' — the same field meaning two different things depending on
        // which path wrote it.
        const trimmed = input[col] === null ? '' : String(input[col]).trim();
