...
- **A platform limitation written into a comment keeps asserting itself after it stops being true.** Both system bars have now been fixed twice, and both times the second fix had to overturn a confident sentence left by the first. `b07026a` collapsed a scheme-scoped `theme-color` pair into one because "the app's theme is a MANUAL toggle" — true then, false from `6bd9079`, which made the theme follow the OS. `cb11749` forced `.mob-nav` to `#000` because "Android paints that bar itself and a PWA cannot colour it" — true then, false from Chrome 135, which extends the viewport into the gesture navigation bar so the app's own safe-area padding paints those pixels. In both cases the comment read as a permanent fact about the platform and was really a dated observation, so the next person inherits a rule whose reason has quietly expired. **Date the claim, not just the code:** say which version or which commit made it true, so the next reader knows what to re-check. And when a fix targets system chrome, confirm the platform still behaves that way before building on it — this is also the one area the repo cannot self-verify, since neither bar reproduces outside an installed PWA on a real handset.
