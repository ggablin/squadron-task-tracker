...
- **Rollout-feedback batch (2026-08-10, from the first partial rollout):**
  ...
  - **Present-vs-all percentages:** an "All members / At drill" toggle on the Squadron stats pane re-renders gauge, shop chart, category card, and most-behind list. Presence rule lives in `lib/presence.js`: a member is present unless attendance rows exist for the cycle and none is `present`/`agr_at_orders` (`orders_away`, added Sep 2026, pays X like orders but is deliberately NOT present: it is how a supervisor says "at tech school / on orders elsewhere") — unmarked counts present, so both numbers match until attendance is marked. Both flavors ship in the same payloads (`*_present` columns on `/api/squadron` + `/api/squadron/categories`; `/api/squadron/members?present=true` is its own top-10).
