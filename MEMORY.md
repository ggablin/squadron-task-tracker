...
- **Rollout-feedback batch (2026-08-10, from the first partial rollout):**
  ...
  - **Present-vs-all percentages:** an "All members / At drill" toggle on the Squadron stats pane re-renders gauge, shop chart, category card, and most-behind list. Presence rule lives in `lib/presence.js`: a member is present unless attendance rows exist for the cycle and none is `present`/`agr_at_orders`/`equiv_training` (Equivalent Training counts as at drill since 2026-09-13: points-only pay, but on site; `orders_away`, added Sep 2026, pays X like orders but is deliberately NOT present: it is how a supervisor says "at tech school / on orders elsewhere") — unmarked counts present, so both numbers match until attendance is marked. Both flavors ship in the same payloads (`*_present` columns on `/api/squadron` + `/api/squadron/categories`; `/api/squadron/members?present=true` is its own top-10).
...
### 2026-09-15 — Squadron chat

In-app group messaging, replacing nothing — it sits alongside the existing bell/push notifications as a new fifth nav tab (My Tasks/My Shop/Squadron/Resources/**Chat**). Three channel types (`channels.type` CHECK'd to `squadron`/`leadership`/`shop`): one squadron-wide channel, one leadership-only channel, and one per shop — seed-on-create like `additional_duties`/`drill_dates`/`calendar_events` (§5), so the boot that first finds `channels` absent inserts the two singletons plus one row per existing shop and never touches it again. Access is symmetric (`lib/chat.js`'s `canPost` is `canAccess`, unchanged — nothing is read-only in v1): every member reads/posts their own shop's channel and the squadron channel; leadership additionally reaches every shop channel and the leadership channel. **Moderation is leadership-only** — `canHide` checks `member.role === 'leadership'` specifically, so a shop supervisor cannot hide a message even in their own shop's channel; a hidden message stays in the table (`messages.hidden_at`/`hidden_by_id`) instead of being deleted, but `listMessages` omits it entirely for anyone who isn't leadership — the row is just absent, not shown-as-hidden. Only leadership sees a "Message hidden by leadership" placeholder where it was. **Push bypasses the `notifications` table entirely** — unlike every other push type (`PUSH_TYPES` in `lib/push.js`: `tasks_live`/`task_assigned`/`task_escalated`, flushed on a cron), a new chat message calls `pushChatMessage()` in `server.js`, which hands recipient ids straight to `lib/push.js`'s `pushToMembers` from the POST-message route (`setImmediate`, mirroring `notify()`'s fire-and-forget shape) — no queued row, no flush cron involved. While a channel is open, new messages arrive by 12s polling (`public/chat.js`), not WebSockets/SSE; push covers the app-backgrounded/closed case. `public/chat.js` follows the `dutiesInit`/`calendarInit` precedent exactly: one global (`chatInit({ canHide })`) called from `switchView()`, a `.reset()` wired into `doLogout()`, and a `badgeCheck()` piggybacked on the existing `updateNavBadges()` — which fires on page load, the browser's `online` event, switching to My Tasks, and after a task mutates, not on a timer — rather than adding a second timer of its own. Full design rationale, schema, and API surface: `docs/superpowers/specs/2026-09-15-squadron-chat-design.md` and `docs/superpowers/plans/2026-09-15-squadron-chat.md`; task-by-task briefs/reports (all 7 tasks — schema, access control, five `/api/chat/*` routes, push, and this frontend wiring) in `.superpowers/sdd/2026-09-15-squadron-chat/`.
...
- `lib/chat.js`, `public/chat.js` — squadron chat: channel access rules (`canAccess`/`canPost`/`canHide`), message CRUD + moderation, and read tracking (lib); the Chat nav tab UI, one global (`chatInit`), initialised from `switchView()`, polling the open channel every 12s while it's the active view (public).
